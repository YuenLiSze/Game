...
from machine_new import yinyue
import time
...
class UI_interface:#ui界面和游戏界面
    def __init__(self):
        self.ui = QUiLoader().load(r"版本2.0的数据文件/3.0ui")#加载ui
    ...
    def people_by_people(self):
        class Chessboard:  # 设置棋盘
            def __init__(s):
                s.grid_lenght = 38  # 棋盘格子的边长
                s.grid_count = 20  # 格子的数量
                s.start_x = 90  # 棋盘初始点坐标，左上角的坐标
                s.start_y = 50
...
#首先初始化各种音效、音乐。然后再是设置棋盘的各个属性，包括格子数量、边长、初始时左上角的坐标。
            def handle_event(s, e):  #处理信息
                ...
                if (mouse_pos[0] > origin_x and mouse_pos[0] <= origin_x + chessboard_lenght) and (
                    ...
                    if not s.gameover:
                        ...
                        c = int(x / s.grid_lenght)  # 换算出X轴第几格，存储时用得整型
                        y = mouse_pos[1] - origin_y
                        r = int(y / s.grid_lenght)  # 换算出Y轴第几格
                        if s.set_piece(r, c):
                            s.check_win(r, c)
                if 800<=mouse_pos[0]<=1000 and 475<=mouse_pos[1]<=575:
                    s.grid=[]
                    for i in range(s.grid_count):  # 棋盘大小为20*20个格子
                        s.grid.append(list("." * s.grid_count))
                    s.gameover=False
                    s.piece='white'
                if 800<=mouse_pos[0]<=1000 and 600<=mouse_pos[1]<=700:
                    pg.quit()


            def set_piece(s, r, c):#下棋的函数
                if s.grid[r][c] == ".":  # 由于前面将所有地方设置为.  如果该位置为.  则该位置没有棋子
                    s.grid[r][c] = s.piece  #前面设置初始化黑方先手
            ...
            def check_win(s, r, c):
                ...
                if (n_count + s_count + 1 >= 5) or (e_count + w_count + 1 >= 5) or (se_count + nw_count + 1 >= 5) or (
                        ne_count + sw_count + 1 >= 5):   #  判断游戏是否结束
                    s.winner = s.grid[r][c]   #获胜者为黑方或者白方
                    s.gameover = True
            ...
            def draw(s, screen):
                # pg.draw.rect(screen, (255, 255, 255), [s.start_x - s.edge_lenght, s.start_y - s.edge_lenght,  # 画棋盘
        ...
        class Gomoku:
            def __init__(s):
                pg.init()
                s.gun_sound = pg.mixer.Sound("版本2.0的数据文件/落子音效.mp3") #人人模式落子音乐
                s.gun_sound.set_volume(4)  #设置落子音效的音量
                s.win_sound = pg.mixer.Sound("版本2.0的数据文件/victory.mp3")  #最终胜利时的音效
                s.win_sound.set_volume(4) #设置胜利音效的音量
                s.just_one=1
                ...
                s.font = pg.font.Font(u"C:\Windows\Fonts\BAUHS93.TTF", 38)  #设置字体
                s.font2 = pg.font.Font(u"C:\Windows\Fonts\BAUHS93.TTF", 35)  # 设置字体
                s.going = True
            ...
            def draw(s):
                bg = pg.image.load(r"版本2.0的数据文件\23.jpeg")
                s.screen.blit(bg, (0, 0))
                s.screen.blit(s.font2.render("Once again!", True, (0, 0, 0)), (830, 525))
                s.screen.blit(s.font.render("Quit!", True, (0, 0, 0)), (860, 650))
